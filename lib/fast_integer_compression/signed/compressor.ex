defmodule FastIntegerCompression.Signed.Compressor do
  ...
  def compress(lst) do
    size = FastIntegerCompression.Signed.expected_compressed_size(lst)
    arr = :array.new(size, [])
  ...
  defp do_compress(val, acc) do
    encoded = Utils.zigzag_encode(val)
    do_encoded_compress(encoded, acc)
  end

  defp do_encoded_compress(val, {arr, pos}) when val < 1 <<< 7 do
    arr = :array.set(pos, val, arr)

    {arr, pos + 1}
  end

  defp do_encoded_compress(val, {arr, pos}) when val < 1 <<< 14 do
  ...
  defp do_encoded_compress(val, {arr, pos}) when val < 1 <<< 21 do
    v1 = band(val, 0x7F) |> bor(0x80)
  ...
  defp do_encoded_compress(val, {arr, pos}) when val < 1 <<< 28 do
    v1 = band(val, 0x7F) |> bor(0x80)
    v2 = bor(band(val >>> 7, 0x7F), 0x80)
  ...
  defp do_encoded_compress(val, {arr, pos}) do
    v1 = band(val, 0x7F) |> bor(0x80)
    v2 = bor(band(val >>> 7, 0x7F), 0x80)
    v3 = bor(band(val >>> 14, 0x7F), 0x80)
